...
;
 ...
 static function getGlovesCategoryIds()
    {
        $gloves_category_obj = DB::table('par_gmdn_codes')
            ->where('is_glove_id', 1)
            ->get();
        $gloves_categories_ass = convertStdClassObjToArray($gloves_category_obj);
        $gloves_categories_simp = convertAssArrayToSimpleArray($gloves_categories_ass, 'id');
        return $gloves_categories_simp;
    }

    static function belongsToGlovesCategory($gmdn_code_id)
        {
            $glovesrCategoryIDs = self::getGlovesCategoryIds();
            $gmdn_code_id_array = is_array($gmdn_code_id) ? $gmdn_code_id : [$gmdn_code_id];
            $arr_intersect = array_intersect($glovesrCategoryIDs, $gmdn_code_id_array);
            if (count($arr_intersect) > 0) {
                return true;
            } else {
                return false;
    ...
    static function getGlovesCategoryCodes()
    {
        $gloves_category_obj = DB::table('par_gmdn_codes')
            ->where('is_glove_id', 1)
            ->get();
        $gloves_categories_ass = convertStdClassObjToArray($gloves_category_obj);
        $gloves_categories_simp = convertAssArrayToSimpleArray($gloves_categories_ass, 'code');
        return $gloves_categories_simp;
    }

    static function belongsToGlovesCategoryByCode($gmdn_code)
        {
            $glovesrCategoryCodes = self::getGlovesCategoryCodes();
            $gmdn_code_array = is_array($gmdn_code) ? $gmdn_code : [$gmdn_code];
            $arr_intersect = array_intersect($glovesrCategoryCodes, $gmdn_code_array);
            if (count($arr_intersect) > 0) {
                return true;
            } else {
                return false;
 ...
 static function getNamesFromIds($table_name,$param_ids)
    ...
    static function funcSaveOnlineImportExportOtherdetails($application_code, $user_id)
    {
        ...
        $portal_db = DB::connection('portal_db');
        $previous_permitdetails = $portal_db->table('wb_permits_products')
          ->where('application_code', $application_code)
          ->get();
        foreach($previous_permitdetails as $previous_permitdetail){  
            ...
            DB::table('tra_permits_products')->insert($data);
