...
function number_to_alpha($num,$code)
      ...
      public function getSrMedicalHistory (Request $req){
        return $this->onLoadMedicalHistoryDetails($req);
      }

      public function getSrSpreadsheets(Request $request)
      {     
        try {
            $filters = $request->input('filters');
            $adr_category_id = $request->input('adr_category_id');
            $patient_gender_id = $request->input('patient_gender_id');
            $age_group_id = $request->input('age_group_id');
            $sourceofpsur_id =$request->input('sourceofpsur_id');
            $adr_report_type_id = $request->input('adr_report_type_id');
            $adr_reporter_category_id = $request->input('adr_reporter_category_id');
            $application_status = $request->input('application_status');
            $approval_recommendation = $request->input('approval_recommendation');
            $validity_status = $request->input('validity_status');
            $is_reporter_notified = $request->input('is_reporter_notified');
            $start = $request->input('start');
            $limit = $request->input('limit');
              ...
              $qry = DB::table('tra_pv_applications as t1')
                ...
                ->leftJoin('par_sourcesofsafety_alerts as t2b', 't1.sourceofpsur_id', '=', 't2b.id')
                ->leftJoin('par_adr_report_types as t2c', 't1.report_type_id', '=', 't2c.id')
                ->leftJoin('par_adr_reporters_categories as t2d', 't1.adr_reporter_category_id', '=', 't2d.id')               
                ->leftJoin('par_pv_age_groups as t2e', 't1.age_group_id', '=', 't2e.id')
                ->leftJoin('wb_trader_account as t3', 't1.applicant_id', '=', 't3.id')
                ->leftJoin('par_system_statuses as t4', 't1.application_status_id', 't4.id')
                ->leftJoin('par_gender as t6', 't1.gender_id', '=', 't6.id')
                ->leftJoin('par_age_units as t23', 't1.patient_age_unit_id', '=', 't23.id')
                ...
                ->leftJoin('tra_pv_personnel as t16', 't1.application_code', 't16.application_code')
                ->leftJoin('par_titles as t7', 't16.title_id', '=', 't7.id')
                ->select(
                    't1.*',
                    't1.date_added as ReceivedFrom',
                    't1.date_added as ReceivedTo',
                    't2b.name as sourceofpsur_name',
                    't2c.name as adr_report_type',
                    't2d.name as adr_reporter_category',
                    't2e.name as age_group_name',
                    't3.name as applicant_name',
                    't4.name as application_status',
                    't2.name as adr_type',
                    't6.name as gender_name',
                    't1.id as active_application_id',
                    't14.decision_id as approval_decision_id', 
                    't1.treatment as final_recommendation', 
                    't15.id as response_id',
                    't15.response',
                    't15.subject',
                    't2a.name as adr_category',
                    # Reporter Grid
                    DB::raw("(CASE 
                    WHEN t1.application_code IS NOT NULL THEN ( 
                        SELECT GROUP_CONCAT(
                            CONCAT_WS(
                            ', ',
                                CONCAT('Reporter: ', COALESCE(t7.name, CONCAT(t16.first_name, ' ', t16.last_name), '')),
                                CONCAT('Email: ', COALESCE(t16.email_address, '')),
                                CONCAT('Mobile No: ', COALESCE(t16.telephone_no, '')),
                                CONCAT('Address: ', COALESCE(t16.physical_address, ''))
                            ) SEPARATOR '<br/> <br/>'
                        ) 
                        FROM tra_pv_personnel as t16
                        WHERE t16.application_code = t1.application_code
                        )
                        ELSE ''
                    END) AS reporter_grid"),
                    ...
                    DB::raw("(CASE 
                        WHEN t1.application_code IS NOT NULL THEN (
                            SELECT GROUP_CONCAT(
                                CONCAT_WS(
                                    ', ',
                                    CONCAT('Previous Medication: ', COALESCE(tra_pv_drug_history.previous_medication, '')),
                                    CONCAT('Previous Medication (WHODRUG): ', COALESCE(tra_pv_drug_history.previous_medication_whodrug, '')),
                                    CONCAT('Meddra Indication: ', COALESCE(tra_pv_drug_history.indication_meddra, '')),
                                    CONCAT('Meddra Reaction: ', COALESCE(tra_pv_drug_history.reaction_meddra, '')),
                                    CONCAT('Dates: ', DATE_FORMAT(tra_pv_drug_history.start_date, '%Y-%m-%d'), '-', DATE_FORMAT(tra_pv_drug_history.end_date, '%Y-%m-%d'))
                                ) SEPARATOR '<br/> <br/>'
                            ) 
                            FROM tra_pv_drug_history
                            WHERE tra_pv_drug_history.application_code = t1.application_code
                        )
                        ELSE '' 
                    END) AS drug_history_grid"),
                    #Medical History Grid    
                    DB::raw("(CASE 
                    WHEN t1.application_code IS NOT NULL THEN (
                        SELECT GROUP_CONCAT(
                            CONCAT_WS(', ',
                                CONCAT('Relevant History: ', COALESCE(tra_pv_medical_history.relevant_history, '')),
                                tra_pv_medical_history.relevant_medical_history,
                                CONCAT(DATE_FORMAT(tra_pv_medical_history.start_date, '%Y-%m-%d'), '-', DATE_FORMAT(tra_pv_medical_history.end_date, '%Y-%m-%d')),
                                CONCAT('Family History: ', COALESCE(t45.name, '')),
                                CONCAT('Continuing: ', COALESCE(t46.name, ''))
                            ) SEPARATOR '<br/> <br/>'
                        ) 
                        FROM tra_pv_medical_history 
                        LEFT JOIN par_confirmations AS t45 ON tra_pv_medical_history.is_family_history_id = t45.id
                        LEFT JOIN par_confirmations AS t46 ON tra_pv_medical_history.continueing_id = t46.id
                        WHERE tra_pv_medical_history.application_code = t1.application_code
                    ) ELSE '' END) AS medical_history_grid"),
                    #Suspected Drug Grid
                    DB::raw("(CASE 
                    WHEN t1.application_code IS NOT NULL THEN (
                        SELECT GROUP_CONCAT(
                            CONCAT_WS(
                                ', ',
                                CONCAT('Drug Name (WHO): ', COALESCE(tra_pv_suspected_drugs.who_drug_name, '')),
                                CONCAT('Brand Name: ', COALESCE(tra_pv_suspected_drugs.brand_name, '')),
                                CONCAT('Drug Role: ', COALESCE(t88.name, '')),
                                CONCAT('Suspected Ingredient: ', COALESCE(t89.name, '')),
                                CONCAT('Strength: ', COALESCE(tra_pv_suspected_drugs.strength, '')),
                                CONCAT('MAH: ', COALESCE(tra_pv_suspected_drugs.mah_holder, '')),
                                CONCAT(DATE_FORMAT(tra_pv_suspected_drugs.administration_start_date, '%Y-%m-%d'), '-', DATE_FORMAT(tra_pv_suspected_drugs.end_date, '%Y-%m-%d'))
                            ) SEPARATOR '<br/> <br/>'
                        )
                        FROM tra_pv_suspected_drugs
                        LEFT JOIN par_adr_drugrole AS t88 ON tra_pv_suspected_drugs.drug_role_id = t88.id
                        LEFT JOIN par_adr_suspected_ingredients AS t89 ON tra_pv_suspected_drugs.suspected_ingredient_id = t89.id
                        WHERE tra_pv_suspected_drugs.application_code = t1.application_code
                    )
                    ELSE '' 
                    END) as suspected_drug_grid"),
                    #Medical Study Grid
                    DB::raw("(CASE WHEN t1.application_code IS NOT NULL THEN (SELECT GROUP_CONCAT(
                    CONCAT_WS(
                            ', ',
                            CONCAT('Study Name: ', COALESCE(tra_pv_study_information.study_name, '')),
                            CONCAT('Type: ', COALESCE(t77.name, '')),
                            CONCAT('Sponsor: ', COALESCE(tra_pv_study_information.sponsor_name, '')),
                            CONCAT('SReg_No: ', COALESCE(tra_pv_study_information.study_registration_number, ''))
                            ) SEPARATOR '<br/> <br/>'
                    ) FROM tra_pv_study_information LEFT JOIN par_clinical_phases as t77 ON tra_pv_study_information.study_type_id = t77.id
                    WHERE tra_pv_study_information.application_code = t1.application_code)ELSE '' END) as medical_study_grid"),
                    #Test $ Procedures Grid
                    DB::raw("(CASE WHEN t1.application_code IS NOT NULL THEN (SELECT GROUP_CONCAT(
                        CONCAT_WS(
                            ', ',
                            CONCAT('Test Name: ', COALESCE(tra_pv_test_procedures.test_name_medra, '')),
                            CONCAT('Test Result: ', COALESCE(CONCAT_WS(' ', t90.name, tra_pv_test_procedures.no_of_tests,t91.name),'')),
                            CONCAT('Test Result Code: ', COALESCE(t92.name, '')),
                            CONCAT('Low Value: ', COALESCE(CONCAT_WS(' ',tra_pv_test_procedures.normal_low_value,t91.name), '')),
                            CONCAT('High Value: ', COALESCE(CONCAT_WS(' ',tra_pv_test_procedures.normal_high_value,t91.name), '')),
                            CONCAT('Test Result: ', COALESCE(tra_pv_test_procedures.result, ''))
                        ) SEPARATOR '<br/> <br/>'
                    ) FROM tra_pv_test_procedures 
                     LEFT JOIN par_adr_test_result_signs as t90 ON tra_pv_test_procedures.test_logic_id = t90.id
                     LEFT JOIN par_si_units AS t91 ON tra_pv_test_procedures.test_si_unit_id = t91.id
                     LEFT JOIN par_adr_test_code AS t92 ON tra_pv_test_procedures.test_result_code_id = t92.id
                     WHERE tra_pv_test_procedures.application_code = t1.application_code)ELSE '' END) as tests_procedure_grid"),
                );
                $qry->orderBy('t1.application_code','desc');
                ...
                if(validateIsNumeric($adr_category_id)){
                    $qry->where('t1.report_category_id', $adr_category_id);
                }
                if(validateIsNumeric($patient_gender_id)){
                    $qry->where('t1.gender_id',$patient_gender_id);
                }
                if(validateIsNumeric($age_group_id)){
                    $qry->where('t1.age_group_id',$age_group_id);
                }
                if(validateIsNumeric($adr_reporter_category_id)){
                    $qry->where('t1.report_category_id',$adr_reporter_category_id);
                }
                if(validateIsNumeric($adr_report_type_id)){
                    $qry->where('t1.report_type_id',$adr_reporter_category_id);
                }
                if(validateIsNumeric($sourceofpsur_id)){
                    $qry->where('t1.sourceofpsur_id',$sourceofpsur_id);
                }
                if(validateIsNumeric($application_status)){
                    $qry->where('t1.application_status_id',$application_status);
                }
                if(validateIsNumeric($is_reporter_notified)){
                    $qry->where('t1.is_reporter_notified',$is_reporter_notified);
                }
                ...
                if(isset($filter)){
                    ...
                    if($filters != NULL){
                        foreach ($filters as $filter){
                            switch($filter->property){
                                case 'reporter_name':
                                    $whereClause[] = "t16.reporter_name like '%" . ($filter->value) . "%'";
                                    break;
                                case 'reporter_email':
                                    $whereClause[] = "t16.reporter_email like '%" . ($filter->value) . "%'";
                                    break;
                                case 'patient_name':
                                    $whereClause[] = "t1.patient_name like '%" . ($filter->value) . "%'";
                                    break;
                                case 'tracking_no':
                                    $whereClause[] = "t1.tracking_no like '%" . ($filter->value) . "%'";
                                    break;
                                case 'patient_height':
                                    $whereClause[] = "t1.patient_height like '%" . ($filter->value) . "%'";
                                    break;
                                case 'bmi':
                                    $whereClause[] = "t1.bmi like '%" . ($filter->value) . "%'";
                                    break;
                                case 'patient_weight':
                                    $whereClause[] = "t1.patient_weight like '%" . ($filter->value) . "%'";
                                    break;
                                case 'date_of_birth':
                                    $whereClause[] = "DATE_FORMAT(t1.date_of_birth, '%Y-%m-%d')= '" . formatDate($filter->value) . "'";
                                    break;
                                case 'initial_receive_date':
                                    $whereClause[] = "DATE_FORMAT(t1.initial_receive_date, '%Y-%m-%d')= '" . formatDate($filter->value) . "'";
                                    break;
                                case 'report_date':
                                    $whereClause[] = "DATE_FORMAT(t1.report_date, '%Y-%m-%d')= '" . formatDate($filter->value) . "'";
                                    break;
                                case 'ReceivedFrom':
                                    $whereClause[] = "DATE_FORMAT(t1.date_added, '%Y-%m-%d')= '" . formatDate($filter->value) . "'";
                                    break;
                                case 'ReceivedTo':
                                    $whereClause[] = "DATE_FORMAT(t1.date_added, '%Y-%m-%d')= '" . formatDate($filter->value) . "'";
                                    break;
                    ...
                    if (!empty($whereClause)) {
                        $filter_string = implode(' AND ', $whereClause);
                    }
                }
                if ($filter_string != ''){
                    $qry->whereRaw($filter_string);
                ...
                function addIndexToGridColumns($results, $placeInObject){
                    foreach($results as $res){
                        $grids= explode('<br/> <br/>', $res->$placeInObject);

                        foreach ($grids as $index => &$grid) {
                            if ($grid && !preg_match('/^\d+: /', $grid)){   
                                #Check if it doesn't already start with a number and colon
                                $grid = ($index + 1) . ': ' . $grid;
                            }
                        }
                        $res->$placeInObject = implode('<br/> <br/>', $grids);
                        $results[] = $res;
                    }
                    return $results;
                }     
                
                $grid_names_in_object = ['reporter_grid','medical_history_grid', 'medical_study_grid', 'drug_history_grid', 'suspected_drug_grid', 'tests_procedure_grid'];
                
                foreach ($grid_names_in_object as $grid_name){
                    $results = addIndexToGridColumns($results, $grid_name);
                }
    ...
    public function getGvpSpreadSheet(request $request){
        try{
               $filters = $request->input('filters');
               $assessment_type = $request->input('assessment_type');
               $facility = $request->input('gvp_type');
               $business_type_id = $request->input('business_type_id');
               $gvp_inspection_id = $request->input('gvp_inspection');
               $issueplace = $request->input('issueplace');
               $decision_id=$request->input('decision');
               $device_type_id = $request->input('device_type_id');
               $start = $request->input('start');
             ...
             $qry= DB::table('tra_gvp_applications as t1')
                   ->LeftJoin('par_gvp_assessment_types as t2','t1.assessment_type_id','t2.id')
                   ...
                   ->leftJoin('tra_personnel_information as t12', 't1.contact_person_id', '=', 't12.id')
                   ->LeftJoin('par_countries as t14','t10.country_id','t14.id')
                   ->LeftJoin('par_regions as t15','t10.region_id','t15.id')
                   ->LeftJoin('par_countries as t16','t11.country_id','t16.id')
                   ->LeftJoin('par_regions as t17','t11.region_id','t17.id')
                  ...
                  ->LeftJoin('par_validity_statuses as t24','t18.appvalidity_status_id','t24.id')
                  ->LeftJoin('par_system_statuses as t25','t1.application_status_id','t25.id')
                ->select('t1.gvp_site_id','t1.reference_no','t1.application_code','t1.tracking_no','t2.name as assessment_procedure','t3.name as gvp_site','t3.physical_address','t3.email_address', 't3.telephone_no','t4.name as gvp_location', 't4.name as gvp_type','t5.name as country','t6.name as region','t7.name as district', 't8.id as gvp_inspection_id', 't8.name as gvp_inspection','t9.name as issueplace','t10.name as Applicant','t10.physical_address as ApplicantPhysicalA','t10.postal_address as ApplicantPostalA','t10.telephone_no as ApplicantTell','t10.mobile_no as ApplicantMobile','t10.email_address as ApplicantEmail','t14.name as ApplicantCountry','t15.name as ApplicantRegion','t11.name as LocalAgent','t11.postal_address as LocalAgentPostalA','t11.physical_address as LocalAgentPhysicalA','t11.telephone_no as LocalAgentTell','t11.mobile_no as AgentMobile','t11.email_address as LocalAgentEmail','t16.name as AgentCountry','t17.name as AgentRegion','t12.name as contact_person','t12.postal_address as contact_personPostalA','t12.telephone_no as contact_personTell','t18.expiry_date as CertExpiryDate','t18.certificate_issue_date as CertIssueDate','t19.name as DeviceType','t18.certificate_issue_date as IssueFrom','t18.certificate_issue_date as IssueTo','t1.date_added as ReceivedFrom','t1.date_added as ReceivedTo', 't18.permit_no', 't23.name as registration_status','t24.name as validity_status', 't21.name as approval_recommendation', 't25.name as application_status');
                ...
                //filters for combo
                    ...
                    if(validateIsNumeric($gvp_inspection_id)){
                        $qry->where('t1.gvp_activities_id', $gvp_inspection_id);
                    }
                    if(validateIsNumeric($facility)){
                          $qry->where('t1.gvp_type_id' , $facility);
                         }
                    if(validateIsNumeric($business_type_id)){
                          $qry->where('t3.business_type_id' , $business_type_id);
                         }
        ...
        public function getGvpSiteProducts(request $request){
            try{
               $filters = $request->input('filters');
               $qry= DB::table('tra_product_gvpinspectiondetails as t1')
               ->LeftJoin('tra_product_information as t2','t1.product_id','t2.id')
               ->LeftJoin('par_atc_codes as t3','t2.common_name_id','t3.id')
               ->LeftJoin('par_classifications as t4','t2.classification_id','t4.id')
               ->LeftJoin('tra_product_applications as t5','t2.id','t5.product_id')
               ->leftJoin('wb_trader_account as t6','t5.applicant_id','t6.id')
    
              ->select('t2.brand_name','t3.description as common_name','t4.name as classification','t5.reference_no','t6.name as Trader','t6.telephone_no as TraderTell');
     ...
     public function getGVPInspectionTeam(Request $req){
        $application_code = $req->application_code;
        try{
            $qry = DB::table('tra_gvp_applications as t1')
                  ->join('assigned_gvpinspections as t9','t1.application_code', 't9.application_code')
                  ...
                  ->select(DB::raw("t9.id, t10.inspectioncountry_list, t10.inspectionteam_name, t10.inspectionteam_desc, t8.name as inspection_type, DATE_FORMAT(t10.start_date,'%d/%m/%Y') as start_date,DATE_FORMAT(t10.end_date, '%d/%m/%Y') as end_date"))
                  ->where('t1.application_code', $application_code);
            $results =$qry->get();
            $res = array(
                      'success' => true,
