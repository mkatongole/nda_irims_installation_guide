	...
	public function updatePremiseOnlineApplicationDetailsOnMIS($request,$application_code,$sub_module_id,$module_id,$view_id,$next_stage)
    {
        ...
        try {
           ...
           $premise_personneldetails = unsetPrimaryIDsInArray($premise_personneldetails, 'id');
			...
			$premise_storedetails = DB::table('wb_premises_storelocation')
                ->where('premise_id', $results->premise_id)
                ->select(DB::raw("country_id,region_id, district_id,$premise_id as premise_id,cell_id,sector_id,village,physical_address,$user_id as created_by"))
                ->get();
...
$user_id =0;
            ...
            $premise_otherdetails = DB::table('wb_premises_otherdetails')
                ->where('premise_id', $results->premise_id)
                ->select(DB::raw("id as portal_id,$premise_id as premise_id,business_type_id,product_category_id, product_details,manufacturing_activities,product_subcategory_id,business_type_detail_id,$user_id as created_by"))
                ->get();
            $premise_otherdetails = convertStdClassObjToArray($premise_otherdetails);
            $premise_otherdetails = unsetPrimaryIDsInArray($premise_otherdetails,'id');
            DB::connection('mis_db')->table('tra_premises_otherdetails')
            ...
            $premise_personneldetails = DB::table('wb_premises_personnel')
                ->where('premise_id', $results->premise_id)
                ->select(DB::raw("id as portal_id,personnel_name,telephone_no, email_address,$premise_id as premise_id,personnel_id,position_id,personnel_identification_no,identification_type_id,qualification_id,start_date,end_date,status_id,professional_board,$user_id as created_by,
                         registration_no,study_field,institution"))
                ->get();
            $premise_personneldetails = convertStdClassObjToArray($premise_personneldetails);
            $premise_personneldetails = unsetPrimaryIDsInArray($premise_personneldetails, 'id');
