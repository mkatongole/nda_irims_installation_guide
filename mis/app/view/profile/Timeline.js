...
Ext.define('Admin.view.profile.Timeline', {
    ...
    cls: 'timeline-items-wrap',
    maxHeight: Ext.Element.getViewportHeight() - 230,
    scrollable: 'y',
    margin: '3 0 0 0',
    //bind: '{userTimeline}',
    store: 'usermessages_str',
    itemSelector: '.timeline-item',
    config : {
        loadMask: false
        //loadingText: ''
    ...
    itemTpl: [
        '<div class="timeline-item{sender_id:this.cls(values,parent[xindex-2],xindex-1,xcount)}" style="width: 580px">' +
        '{date:this.epoch(values,parent[xindex-2],xindex-1,xcount)}' +
        '<div class="profile-pic-wrap">' +
        '<img src="{profile_pic:this.hasValue}" alt="Profile Picture">' +
        '<div>{date:this.elapsed} ago</div>' +
        '</div>' +
        '<tpl if="sender_id == ' + user_id + '">' +
        '<div class="line-wrap">' +
        '<div class="contents-wrap">' +
        '<div class="shared-by"><a href="#" style="color: #32404E">To {recipient_fname} {recipient_lname}</a></div>' +
        '<div class="article-comment">{message}</div>' +
        '</div>' +
        '</div>' +
        '<tpl else>' +
        '<div class="line-wrap">' +
        '<div class="contents-wrap">' +
        '<div class="job-meeting"><a href="#">From {sender_fname} {sender_lname}</a></div>' +
        '<div>{message}</div>' +
        '</div>' +
        '</div>' +
        '</tpl>' +
