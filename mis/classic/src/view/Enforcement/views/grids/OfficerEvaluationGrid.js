Ext.define('Admin.view.Enforcement.views.grids.OfficerEvaluationGrid', {
    extend: 'Ext.grid.Panel',
    xtype: 'officerEvaluationGrid',
    ...
    viewConfig: {
		...
		getRowClass: function (record, rowIndex, rowParams, store) {
            var investigation_decision_id = record.get('investigation_decision_id');
			if(investigation_decision_id == 1){
				if (investigation_decision_id > 0 ) {
					return 'valid-row';
				}
			}
           else{
        ...
        deselect: function (sel, record, index, eOpts) {
		 ...
		 beforeselect: function(sel, record, index, eOpts) {
            var me = this,
                grid = sel.view.grid,
                panel = grid.up('panel'),
                investigation_decision_id = record.get('investigation_decision_id'),
                selCount = grid.getSelectionModel().getCount();
            if (investigation_decision_id == 1 || investigation_decision_id == 2) {
            	if(selCount > 0 ){
            		panel.down('button[name=process_submission_btn]').setDisabled(false);
            	}else{
					panel.down('button[name=process_submission_btn]').setDisabled(true);
    ...
    columns: [{
		...
		{
	        xtype: 'gridcolumn',
	        dataIndex: 'investigation_decision_id',
	        ...
	        widget: {
	            ...
	            menu: {
	                ...
	                },
					{
	                    ...
	                    tooltip: 'Decision',
	                   	action: 'edit',
						childXtype: 'investigationDecisionFrm',
						winTitle: 'Investigation Desicion',
						winWidth: '50%',
	                    handler: 'showInvestigationApprovalwin',
	                    stores: '[]'
	                }
	                ]
	            }
