...
@Component({
  selector: 'app-online-navigation',
  templateUrl: './online-navigation.component.html',
  styleUrls: ['./online-navigation.component.css']
})
export class OnlineNavigationComponent implements OnInit {
  ...
  pieChartData: any[]=[];
  chartData: any[] = [];
  cssClassField = 'cssClass'; // The name of the CSS class property in your data
  customPalette: string[] = ['#FFA500', '#FF5D48', '#1BB99A', '#FF5D48', '#64B0F2'];
 ...
 this.FilterDetailsFrm = new FormGroup({
          ...
          if (data.success) {
            let records = data.data;
             ...
             for(let rec of records){
                 ...
                 this.chartData = [
                    { status: 'Pending Submission', count: this.pending_submission, cssClass: 'pending-class' },
                    { status: 'Queried Applications', count: this.queried_applications, cssClass: 'queried-class' },
                    { status: 'Approved Applications', count: this.approved_applications, cssClass: 'approved-class' },
                    { status: 'Rejected Applications', count: this.rejected_applications, cssClass: 'rejected-class' },
                    { status: 'Invoiced Pending Payment', count: this.invoiced_pendingpayment, cssClass: 'invoiced-class' },
                  ];

                 this.pieChartData = [
                    { status: 'Pending Submission', count: this.pending_submission, color: this.getStatusColor('Pending Submission') },
                    { status: 'Queried Applications', count: this.queried_applications, color: this.getStatusColor('Queried Applications') },
                    { status: 'Approved Applications', count: this.approved_applications, color: this.getStatusColor('Approved Applications') },
                    { status: 'Rejected Applications', count: this.rejected_applications, color: this.getStatusColor('Rejected Applications') },
                    { status: 'Invoiced Pending Payment', count: this.invoiced_pendingpayment, color: this.getStatusColor('Invoiced Pending Payment') },
                  ];
  ...
  const index = this.pieChartData.findIndex(item => item.status === status);
  if (index !== -1) {
    return this.customPalette[index];
  }
  return '#000'; // Default color if status is not found
}
