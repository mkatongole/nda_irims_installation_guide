...
@Component({
  selector: 'app-preinspection-gmp-application',
  templateUrl: './preinspection-gmp-application.component.html',
  styleUrls: ['./preinspection-gmp-application.component.css']
})
export class PreinspectionGmpApplicationComponent extends SharedGmpapplicationclassComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (!this.gmpapp_details) {
      this.router.navigate(['./../online-services/gmp-preinspection-dashboard']);
      return;
    }
    else {
    ...
    if (this.is_local == 1) {
      ...
      this.gmppreInspectionGeneraldetailsfrm.patchValue({ local_agent_name: this.trader_name, local_agent_id: this.trader_id, trader_aslocal_agent: 1 })
      //deiable field
    }
    else {
      this.localagent_optionDisabled = false;
      this.gmppreInspectionGeneraldetailsfrm.patchValue({ local_agent_name: 'Select Local Agent', local_agent_id: '', trader_aslocal_agent: 0 })
  ...
  onSavePreGMPApplication() {
    if (this.gmppreInspectionGeneraldetailsfrm.invalid) {
      return;
    }
   
   this.spinner.show();
    this.appService.onSaveGmpApplication(this.manufacturing_site_id, this.gmppreInspectionGeneraldetailsfrm.value, this.tracking_no,'')
      .subscribe(
        response => {
