...
@Component({
  selector: 'app-preinspection-reg-dashboard',
  templateUrl: './preinspection-reg-dashboard.component.html',
  styleUrls: ['./preinspection-reg-dashboard.component.css']
})
@NgModule({
  imports: [
    DxDataGridModule,
...
export class PreinspectionRegDashboardComponent extends SharedDashboardclassComponent implements OnInit {

  ngOnInit() {
    this.onLoadSections();
    this.sub_module_id = 117;
  ...
  onClickSubModuleAppSelection(sub_module_id,sub_module_name){
    ...
    if(this.sub_module_id){
      this.app_route = ['./online-services/gmp-preinspection-selection'];
