...
@Component({
  selector: 'app-control-excipient',
  templateUrl: './control-excipient.component.html',
  styleUrls: ['./control-excipient.component.css']
})
export class ControlExcipientComponent extends SharedProductregistrationclassComponent{
 ...
 // @Input() summaryProductDetailsFrm:FormGroup;
  excipientControlFrm:FormGroup;
  analyticalProcedureFrm:FormGroup;
  validationProcedureFrm
  justificationFrm:FormGroup;
  humanAnimalFrm:FormGroup;
  novelExcipientsFrm:FormGroup;

  novelExcipientData:any;
  animalHumanData:any;
  justSpecificData:any;
  validityData:any;
  analyticsData:any;
  excipientControlData:any;
  ...
  excipientControlWin:boolean = false;
  analyticalProcedureWin:boolean = false;
  validationProcedureWin:boolean = false;
  justificationSpecificWin:boolean = false;
  humanAnimalWin:boolean = false;
  novelExcipientsWin:boolean = false;
  ...
  isSummaryProductDetailsWinshow:boolean=false;
 ...
 ngOnInit() { 
    ...
    this.excipientControlFrm = new FormGroup({
    ...
    this.analyticalProcedureFrm = new FormGroup({
      analytical_procedure: new FormControl('', Validators.compose([Validators.required])),
      id: new FormControl('', Validators.compose([]))
    });
    this.validationProcedureFrm = new FormGroup({
      validation_analytical_procedures: new FormControl('', Validators.compose([Validators.required])),
      id: new FormControl('', Validators.compose([]))
    });

    this.justificationFrm = new FormGroup({
      justification_test: new FormControl('', Validators.compose([Validators.required])),
      id: new FormControl('', Validators.compose([]))
    });
    this.humanAnimalFrm = new FormGroup({
      letter_attestation: new FormControl('', Validators.compose([Validators.required])),
      tse_compliance: new FormControl('', Validators.compose([Validators.required])),
      id: new FormControl('', Validators.compose([]))
    });

    this.novelExcipientsFrm = new FormGroup({
      novel_excipients: new FormControl('', Validators.compose([Validators.required])),
      id: new FormControl('', Validators.compose([]))
    });
    ...
    this.OnLoadExcipientControl(product_id);
    this.OnLoadProductsPackagingMaterials(product_id);    
    this.OnLoadAnalytics(product_id);    
    this.OnLoadVallidity(product_id);    
    this.OnLoadJustification(product_id);    
    this.OnLoadHumanAnimalExcipient(product_id);
    this.OnLoadNovelExcipient(product_id);
  ...
  onExcipientControl(e) {
    this.functDataGridToolbar(e, this.funcAddExcipientControl, 'Excipient Control');
  }

  onAnalyticalPreparing(e) {
    this.functDataGridToolbar(e, this.funcAddAnalytical, 'Analytical Procedures');
  }
  onValidationPreparing(e) {
    this.functDataGridToolbar(e, this.funcAddValidation, 'Validation Procedures');
  }
  onJustificationPreparing(e) {
    this.functDataGridToolbar(e, this.funcAddJustification, 'Justification of Specifications ');
  }
  onHumanAnimalPreparing(e) {
    this.functDataGridToolbar(e, this.funcAddHumanAnimal, 'Excipients of Human Origin ');
  }
  onNovelExcipientPreparing(e) {
    this.functDataGridToolbar(e, this.funcAddNovelExcipient, 'Novel Excipients  ');
  }
  
  funcAddExcipientControl() {
    this.excipientControlWin = true;
    this.excipientControlFrm.reset();

  }
  funcAddAnalytical() {
    this.analyticalProcedureWin = true;
    this.analyticalProcedureFrm.reset();

  }
  funcAddValidation() {
    this.validationProcedureWin = true;
    this.validationProcedureFrm.reset();

  }
  funcAddJustification() {
    this.justificationSpecificWin = true;
    this.justificationFrm.reset();

  }
  funcAddHumanAnimal() {
    this.humanAnimalWin = true;
    this.humanAnimalFrm.reset();

  }  
  funcAddNovelExcipient() {
    this.novelExcipientsWin = true;
    this.novelExcipientsFrm.reset();
          ...
          this.appService.onDeleteProductsDetails(record_id, table_name, appproduct_id, title)
            ...
            .subscribe(
              data_response => {
                ...
                if (resp.success) {
                 if (reload_type == 'exicipients') {
                    this.OnLoadExcipientControl(appproduct_id);
                  }
                  else if (reload_type == 'analytics') {
                    this.OnLoadAnalytics(appproduct_id);

                  }else if (reload_type == 'validity') {
                    this.OnLoadVallidity(appproduct_id);

                  } else if (reload_type == 'justification_specific') {

                    this.OnLoadJustification(appproduct_id);
                  }else if (reload_type == 'human_animal') {

                    this.OnLoadHumanAnimalExcipient(appproduct_id);
                  }else if (reload_type == 'novel_excipient') {

                    this.OnLoadNovelExcipient(appproduct_id);
                  }    
                  this.spinner.hide();
...
OnLoadExcipientControl(product_id) {
    var data = {
      table_name: 'tra_excipients_controls',
      product_id: product_id
    };
    this.config.getProductsQualitySummaryDetails(data)
      .subscribe(
        data => {
          this.excipientControlData = data.data;
        });
  }
OnLoadAnalytics(product_id) {
    var data = {
      table_name: 'tra_analytics',
      product_id: product_id
    };
    this.config.getProductsQualitySummaryDetails(data)
      .subscribe(
...
OnLoadVallidity(product_id) {
    var data = {
      table_name: 'tra_validity',
      product_id: product_id
    };
    this.config.getProductsQualitySummaryDetails(data)
      .subscribe(
        data => {
          this.validityData = data.data;
        });
  }
  OnLoadJustification(product_id) {
    var data = {
      table_name: 'tra_justificationspecification',
      product_id: product_id
    };
    this.config.getProductsQualitySummaryDetails(data)
      .subscribe(
        data => {
          this.justSpecificData = data.data;
        });
  }
  OnLoadHumanAnimalExcipient(product_id) {
    var data = {
      table_name: 'tra_hunamanimal_excipient',
      product_id: product_id
    };
    this.config.getProductsQualitySummaryDetails(data)
      .subscribe(
        data => {
          this.animalHumanData = data.data;
        });
  }
  OnLoadNovelExcipient(product_id) {
    var data = {
      table_name: 'tra_novel_excipient',
      product_id: product_id
    };
    this.config.getProductsQualitySummaryDetails(data)
      .subscribe(
        data => {
          this.novelExcipientData = data.data;
        });
  }
...
onSaveExcipientControlDetails() {
  let appproduct_id = this.product_id;
  this.spinner.show();
  this.appService.onSaveProductQualitySummaryDetails('tra_excipients_controls', this.excipientControlFrm.value, this.product_id,this.application_code)
    .subscribe(
      response => {
        this.product_resp = response.json();
        //the details 
        if (this.product_resp.success) {
          ...
          this.excipientControlWin = false;
          this.toastr.success(this.product_resp.message, 'Response');
        } else {
          this.toastr.error(this.product_resp.message, 'Alert');
        }
...
onSaveAnalyticalProcedureDetails() {
  let appproduct_id = this.product_id;
  this.spinner.show();
  this.appService.onSaveProductQualitySummaryDetails('tra_analytics', this.analyticalProcedureFrm.value, this.product_id,this.application_code)
    .subscribe(
      response => {
        this.product_resp = response.json();
        //the details 
        if (this.product_resp.success) {
          ...
          this.analyticalProcedureWin = false;
          this.toastr.success(this.product_resp.message, 'Response');
        } else {
          this.toastr.error(this.product_resp.message, 'Alert');
        }
...
onSaveValidationProcedureDetails() {
  let appproduct_id = this.product_id;
  this.spinner.show();
  this.appService.onSaveProductQualitySummaryDetails('tra_validity', this.validationProcedureFrm.value, this.product_id,this.application_code)
    .subscribe(
      response => {
        this.product_resp = response.json();
        //the details 
        if (this.product_resp.success) {
          ...
          this.validationProcedureWin = false;
          this.toastr.success(this.product_resp.message, 'Response');
        } else {
          this.toastr.error(this.product_resp.message, 'Alert');
        }
...
onSaveJusificationDetails() {
  let appproduct_id = this.product_id;
  this.spinner.show();
  this.appService.onSaveProductQualitySummaryDetails('tra_justificationspecification', this.justificationFrm.value, this.product_id,this.application_code)
    .subscribe(
      response => {
        this.product_resp = response.json();
        //the details 
        if (this.product_resp.success) {
          ...
          this.justificationSpecificWin = false;
          this.toastr.success(this.product_resp.message, 'Response');
        } else {
          this.toastr.error(this.product_resp.message, 'Alert');
        }
...
onSaveHumanAnimalOriginDetails() {
  let appproduct_id = this.product_id;
  this.spinner.show();
  this.appService.onSaveProductQualitySummaryDetails('tra_hunamanimal_excipient', this.humanAnimalFrm.value, this.product_id,this.application_code)
    .subscribe(
      response => {
        this.product_resp = response.json();
        //the details 
...
onSaveNovelExcipientDetails() {
  let appproduct_id = this.product_id;
  this.spinner.show();
  this.appService.onSaveProductQualitySummaryDetails('tra_novel_excipient', this.novelExcipientsFrm.value, this.product_id,this.application_code)
    .subscribe(
      response => {
        this.product_resp = response.json();
        //the details 
