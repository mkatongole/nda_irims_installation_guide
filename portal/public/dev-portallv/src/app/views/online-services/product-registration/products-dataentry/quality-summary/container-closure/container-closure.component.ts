...
@Component({
  selector: 'app-container-closure',
  templateUrl: './container-closure.component.html',
  styleUrls: ['./container-closure.component.css']
})
export class ContainerClosureComponent extends SharedProductregistrationclassComponent{
 ...
 ngOnInit() { this.onLoadconfirmDataParmAll();
    ...
    this.containerClosureDetailsFrm = new FormGroup({
      summary_primarysecondary_packaging: new FormControl('', Validators.compose([Validators.required])),
      description_container_closure: new FormControl('', Validators.compose([Validators.required])),
      other_information: new FormControl('', Validators.compose([Validators.required])),
  ...
  autoLoadProductsOtherDetails(product_id) {
    this.OnLoadContainerClosure(product_id);
            ...
            .subscribe(
              data_response => {
                ...
                if (resp.success) {
                    ...
                    this.OnLoadContainerClosure(appproduct_id);
                  this.spinner.hide();
...
OnLoadContainerClosure(product_id) {
    var data = {
      table_name: 'tra_container_closuresystem',
...
onSaveContainerClosureDetails() {
  let appproduct_id = this.product_id;
  this.spinner.show();
  this.appService.onSaveProductQualitySummaryDetails('tra_container_closuresystem', this.containerClosureDetailsFrm.value, this.product_id,this.application_code)
    .subscribe(
      response => {
