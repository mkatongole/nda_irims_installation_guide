...
export class ProductSummaryComponent extends SharedProductregistrationclassComponent{
 ...
 ngOnInit() { 
    ...
    this.summaryProductDetailsFrm = new FormGroup({
      non_proprietary_name: new FormControl('', Validators.compose([Validators.required])),
      ...
      contact_information: new FormControl('', Validators.compose([Validators.required]))
    });
    this.onLoadProductApplciation(this.application_code);
    ...
    this.OnLoadProductsSummary(product_id);
 ...
 onLoadProductApplciation(application_code) {

  this.appService.getProductsOtherDetails({ application_code: application_code }, 'getProductApplicationInformation')
    //.pipe(first())
    .subscribe(
      data => {
        if (data.success) {
          this.ProductApplicationData = data.data;
        }
        else {
  ...
  onProdSummaryPreparing(e) {
    this.functDataGridToolbar(e, this.funcAddProductSummary, 'Summary Product information');
  }
funcAddProductSummary() {
  this.onLoadProductApplciation(this.application_code);
  this.isSummaryProductDetailsWinshow = true;
  this.summaryProductDetailsFrm.reset();
  if (this.ProductApplicationData) {
      this.summaryProductDetailsFrm.patchValue(this.ProductApplicationData);
          ...
          this.appService.onDeleteProductsDetails(record_id, table_name, appproduct_id, title)
            ...
            .subscribe(
              data_response => {
                ...
                if (resp.success) {
                  this.OnLoadProductsSummary(appproduct_id);
                  this.spinner.hide();
...
OnLoadProductsSummary(product_id) {
    var data = {
      table_name: 'tra_product_summary_information',
      product_id: product_id
    };
    this.config.getProductsQualitySummaryDetails(data)
      .subscribe(
        data => {
          this.drugsSummaryData = data.data;
        });
  }
...
onSavesummaryProductDetails() {
  let appproduct_id = this.product_id;
  this.spinner.show();
  this.appService.onSaveProductQualitySummaryDetails('tra_product_summary_information', this.summaryProductDetailsFrm.value, this.product_id,this.application_code)
    .subscribe(
      response => {
        ...
        if (this.product_resp.success) {
          ...
          this.isSummaryProductDetailsWinshow = false;
