...
@Component({
  selector: 'app-active-pharmaceutical',
  templateUrl: './active-pharmaceutical.component.html',
  styleUrls: ['./active-pharmaceutical.component.css']
})
export class ActivePharmaceuticalComponent extends SharedProductregistrationclassComponent{
 ...
 // @Input() summaryProductDetailsFrm:FormGroup;
  activePharmaceuticalFrm:FormGroup;
  ...
  isActivePharmaceuticalWin:boolean = false;
 ...
 ngOnInit() { 
    ...
    this.activePharmaceuticalFrm = new FormGroup({
      active_ingredient: new FormControl('', Validators.compose([Validators.required])),
      manufacturer: new FormControl('', Validators.compose([Validators.required])),
      full_details_PD: new FormControl('', Validators.compose([Validators.required])),
      cert_suitability: new FormControl('', Validators.compose([Validators.required])),
    ...
    this.OnLoadActiveIngredients(product_id);
  ...
  onActivePharm(e) {
    this.functDataGridToolbar(e, this.funcAddActviePharmaceutical, 'Active Pharmaceutica Ingredients');
  }
  funcAddActviePharmaceutical() {
    this.OnLoadapiManufacturersData(this.product_id);
    this.isActivePharmaceuticalWin = true;
    this.activePharmaceuticalFrm.reset();

if (this.apiManufacturersData && this.apiManufacturersData.length > 0) {
  const concatenatedManufacturerValues = [];
  const concatenatedActiveIngredientValues = [];
  ...
  this.apiManufacturersData.forEach((data, index) => {
    ...
    const concatenatedActiveIngredientValue = `${index + 1}. ${data.active_ingredient}\n`;
    concatenatedActiveIngredientValues.push(concatenatedActiveIngredientValue);
  });

  this.activePharmaceuticalFrm.get('manufacturer').patchValue(concatenatedManufacturerValues);
  this.activePharmaceuticalFrm.get('active_ingredient').patchValue(concatenatedActiveIngredientValues);
}
  }
          ...
          this.appService.onDeleteProductsDetails(record_id, table_name, appproduct_id, title)
            ...
            .subscribe(
              data_response => {
                ...
                if (resp.success) {
                    this.OnLoadActiveIngredients(appproduct_id)
                  this.spinner.hide();
...
OnLoadActiveIngredients(product_id) {
    var data = {
      table_name: 'tra_active__pharmaceutical',
...
onSaveActivePharmaceuticalDetails() {
  ...
  this.spinner.show();
  this.appService.onSaveProductQualitySummaryDetails('tra_active__pharmaceutical', this.activePharmaceuticalFrm.value, this.product_id,this.application_code)
    .subscribe(
      response => {
        ...
        if (this.product_resp.success) {
          ...
          this.isActivePharmaceuticalWin = false;
