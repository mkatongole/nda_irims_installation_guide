...
@Component({
  selector: 'app-stability-p8',
  templateUrl: './stability-p8.component.html',
  styleUrls: ['./stability-p8.component.css']
})
export class StabilityP8Component extends SharedProductregistrationclassComponent{
 ...
 // @Input() summaryProductDetailsFrm:FormGroup;
  ...
  stabilitySummaryWinshow:boolean=false;
  isPostApprovalWinshow:boolean=false;
  isStabilityDataWinshow:boolean=false;
  ...
  stabilitySummaryData:any;
  postApprovalData:any;
  criticalControlData:any;
  stabilityData:any;

  stabilitySummaryDetailsFrm:FormGroup;
  postApprovalDetailsFrm:FormGroup;
  stabilityDataDetailsFrm:FormGroup;
 ...
 ngOnInit() { this.onLoadconfirmDataParmAll();
    ...
    this.stabilitySummaryDetailsFrm = new FormGroup({
      summary_acceleratedtesting_parameters: new FormControl('', Validators.compose([Validators.required])),
      proposed_storage_statement: new FormControl('', Validators.compose([Validators.required])),
      summary_stress_results: new FormControl('', Validators.compose([Validators.required])),
      id: new FormControl('', Validators.compose([]))
    });

    this.postApprovalDetailsFrm = new FormGroup({
      stability_primary_batches: new FormControl('', Validators.compose([Validators.required])),
      stability_committment_batches: new FormControl('', Validators.compose([Validators.required])),
      stability_ongoing_batches: new FormControl('', Validators.compose([Validators.required])),
      id: new FormControl('', Validators.compose([]))
      
    });
    this.stabilityDataDetailsFrm = new FormGroup({
      actual_results: new FormControl('', Validators.compose([Validators.required])),
      summary_analytical_procedure: new FormControl('', Validators.compose([Validators.required])),
      bracket_matrixing_design: new FormControl('', Validators.compose([Validators.required])),
      id: new FormControl('', Validators.compose([]))
  ...
  autoLoadProductsOtherDetails(product_id) {
    this.OnLoadStabilitySummary(product_id);
    this.OnLoadPostApproval(product_id);    
    this.OnLoadStabilityData(product_id);
  ...
  onStabilitySummaryPreparing(e) {
    this.functDataGridToolbar(e, this.funcAddAStability, 'Stability Summary');
  }
  funcAddAStability() {
    
    this.stabilitySummaryWinshow = true;
    this.stabilitySummaryDetailsFrm.reset();

  }
  onPostApprovalPreparing(e) {
    this.functDataGridToolbar(e, this.funcAddPostApproval, 'Post Approval');
  }

  onStabilityDataPreparing(e) {
    this.functDataGridToolbar(e, this.funcAddStabilityData, 'Stability Data');
  }

  funcAddPostApproval() {
    
    this.isPostApprovalWinshow = true;
    this.postApprovalDetailsFrm.reset();

  }

  funcAddStabilityData() {
    this.isStabilityDataWinshow = true;
    this.stabilityDataDetailsFrm.reset();
          ...
          this.appService.onDeleteProductsDetails(record_id, table_name, appproduct_id, title)
            ...
            .subscribe(
              data_response => {
                ...
                if (resp.success) {
                  if (reload_type == "stability_summary") {
                    this.OnLoadStabilitySummary(appproduct_id);

                  }
                  else if (reload_type == 'post_approval') {
                    this.OnLoadPostApproval(appproduct_id);

                  } else if (reload_type == 'stability_data') {
                    this.OnLoadStabilityData(appproduct_id);
...
OnLoadStabilitySummary(product_id) {
    var data = {
      table_name: 'tra_stabilitydossier_summary',
      product_id: product_id
    };
    this.config.getProductsQualitySummaryDetails(data)
      .subscribe(
        data => {
          this.stabilitySummaryData = data.data;
        });
  }

OnLoadPostApproval(product_id) {
    var data = {
      table_name: 'tra_post_approval',
      product_id: product_id
    };
    this.config.getProductsQualitySummaryDetails(data)
      .subscribe(
        data => {
          this.postApprovalData = data.data;
        });
  }

OnLoadStabilityData(product_id) {
    var data = {
      table_name: 'tra_stable_data',
      product_id: product_id
    };
    this.config.getProductsQualitySummaryDetails(data)
      .subscribe(
...
onSaveStabilitySummaryDetails() {
  let appproduct_id = this.product_id;
  this.spinner.show();
  this.appService.onSaveProductQualitySummaryDetails('tra_stabilitydossier_summary', this.stabilitySummaryDetailsFrm.value, this.product_id,this.application_code)
    .subscribe(
      response => {
        this.product_resp = response.json();
        //the details 
        if (this.product_resp.success) {
          ...
          this.stabilitySummaryWinshow = false;
          this.toastr.success(this.product_resp.message, 'Response');
        } else {
          this.toastr.error(this.product_resp.message, 'Alert');
        }
...
onSavePostApprovalDetails() {
  let appproduct_id = this.product_id;
  this.spinner.show();
  this.appService.onSaveProductQualitySummaryDetails('tra_post_approval', this.postApprovalDetailsFrm.value, this.product_id,this.application_code)
    .subscribe(
      response => {
        this.product_resp = response.json();
        //the details 
        if (this.product_resp.success) {
          ...
          this.isPostApprovalWinshow = false;
          this.toastr.success(this.product_resp.message, 'Response');
        } else {
          this.toastr.error(this.product_resp.message, 'Alert');
        }
...
onSaveStabilityDataDetails() {
  let appproduct_id = this.product_id;
  this.spinner.show();
  this.appService.onSaveProductQualitySummaryDetails('tra_stable_data', this.stabilityDataDetailsFrm.value, this.product_id,this.application_code)
    .subscribe(
      response => {
        this.product_resp = response.json();
        //the details 
        if (this.product_resp.success) {
          ...
          this.isStabilityDataWinshow = false;
          this.toastr.success(this.product_resp.message, 'Response');
        } else {
          this.toastr.error(this.product_resp.message, 'Alert');
        }
