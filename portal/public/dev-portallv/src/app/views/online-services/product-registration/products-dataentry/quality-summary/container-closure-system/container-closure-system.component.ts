...
@Component({
  selector: 'app-container-closure-system',
  templateUrl: './container-closure-system.component.html',
  styleUrls: ['./container-closure-system.component.css']
})
export class ContainerClosureSystemComponent extends SharedProductregistrationclassComponent{
 ...
 ngOnInit() { this.onLoadconfirmDataParmAll();
    ...
    this.containerClosureDetailsFrm = new FormGroup({
      description_closure_system: new FormControl('', Validators.compose([Validators.required])),
      other_information: new FormControl('', Validators.compose([])),
...
OnLoadContainerClosure(product_id) {
    var data = {
      table_name: 'tra_container_closure',
...
onSaveContainerClosureDetails() {
  ...
  this.spinner.show();
  this.appService.onSaveProductQualitySummaryDetails('tra_container_closure', this.containerClosureDetailsFrm.value, this.product_id,this.application_code)
    .subscribe(
      response => {
