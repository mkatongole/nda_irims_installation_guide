...
@Component({
  selector: 'app-reference-standard',
  templateUrl: './reference-standard.component.html',
  styleUrls: ['./reference-standard.component.css']
})
export class ReferenceStandardComponent extends SharedProductregistrationclassComponent{
 ...
 // @Input() summaryProductDetailsFrm:FormGroup;
  ...
  isReferenceStandardsWinshow:boolean=false;
  ...
  referenceStandardData:any;
  criticalControlData:any;

  referenceStandardsDetailsFrm:FormGroup;
 ...
 ngOnInit() { this.onLoadconfirmDataParmAll();
    ...
    this.referenceStandardsDetailsFrm = new FormGroup({
      source_reference: new FormControl('', Validators.compose([Validators.required])),
      characterization_evaluation: new FormControl('', Validators.compose([Validators.required])),
      description_process_control: new FormControl('', Validators.compose([Validators.required])),
      id: new FormControl('', Validators.compose([]))
    });

    this.batchAnalysisDetailsFrm = new FormGroup({
      batch_number: new FormControl('', Validators.compose([Validators.required])),
      batch_size: new FormControl('', Validators.compose([Validators.required])),
      date_site_production: new FormControl('', Validators.compose([Validators.required])),
  ...
  autoLoadProductsOtherDetails(product_id) {
    this.OnLoadReferenceStandard(product_id);
  ...
  onReferenceStandardsPreparing(e) {
    this.functDataGridToolbar(e, this.funcAddReferenceStandard, 'Reference standard');
  }
  funcAddReferenceStandard() {
    
    this.isReferenceStandardsWinshow = true;
    this.referenceStandardsDetailsFrm.reset();
            ...
            .subscribe(
              data_response => {
                ...
                if (resp.success) {
                    ...
                    this.OnLoadReferenceStandard(appproduct_id);
                  this.spinner.hide();
...
OnLoadReferenceStandard(product_id) {
    var data = {
      table_name: 'tra_reference_standard',
      product_id: product_id
    };
    this.config.getProductsQualitySummaryDetails(data)
      .subscribe(
        data => {
          this.referenceStandardData = data.data;
        });
  }
...
onSaveReferenceStandardsDetails() {
  let appproduct_id = this.product_id;
  this.spinner.show();
  this.appService.onSaveProductQualitySummaryDetails('tra_reference_standard', this.referenceStandardsDetailsFrm.value, this.product_id,this.application_code)
    .subscribe(
      response => {
        ...
        if (this.product_resp.success) {
          ...
          this.isReferenceStandardsWinshow = false;
