...
export class ReferenceMaterialsComponent extends SharedProductregistrationclassComponent{
 ...
 // @Input() summaryProductDetailsFrm:FormGroup;
  refernceStandardsFrm:FormGroup;
  referenceStandardsData:any;
  ...
  referenceStandardsWin:boolean=false;
 ...
 ngOnInit() { 
    ...
    this.refernceStandardsFrm = new FormGroup({
      description_of_source: new FormControl('', Validators.compose([Validators.required])),
      characterisation_evaluation: new FormControl('', Validators.compose([Validators.required])),
      decription_control: new FormControl('', Validators.compose([Validators.required])),
    ...
    this.OnLoadReferenceStandards(product_id);
  ...
  referenceStandardsControl(e) {
    this.functDataGridToolbar(e, this.funcAddReferenceStandards, 'Reference Standards');
  }
  funcAddReferenceStandards() {
    
    this.referenceStandardsWin = true;
    this.refernceStandardsFrm.reset();
          ...
          this.appService.onDeleteProductsDetails(record_id, table_name, appproduct_id, title)
            ...
            .subscribe(
              data_response => {
                ...
                if (resp.success) {
                    this.OnLoadReferenceStandards(appproduct_id)
                  this.spinner.hide();
...
OnLoadReferenceStandards(product_id) {
    var data = {
      table_name: 'tra_reference_standards',
      product_id: product_id
    };
    this.config.getProductsQualitySummaryDetails(data)
      .subscribe(
        data => {
          this.referenceStandardsData = data.data;
        });
  }
...
onSaveReferenceStandardsDetails() {
  ...
  this.spinner.show();
  this.appService.onSaveProductQualitySummaryDetails('tra_reference_standards', this.refernceStandardsFrm.value, this.product_id,this.application_code)
    .subscribe(
      response => {
        ...
        if (this.product_resp.success) {
          ...
          this.referenceStandardsWin = false;
