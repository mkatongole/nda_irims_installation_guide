...
@Component({
  ...
  styleUrls: ['./stability.component.css']
})
export class StabilityComponent extends SharedProductregistrationclassComponent{
 ...
 // @Input() summaryProductDetailsFrm:FormGroup;
  ...
  isStabilitySummaryWinshow:boolean=false;
 ...
 ngOnInit() { this.onLoadconfirmDataParmAll();
    ...
    this.stabilitySummaryDetailsFrm = new FormGroup({
      longterm_testing_parameter: new FormControl('', Validators.compose([Validators.required])),
      summry_stress_testing: new FormControl('', Validators.compose([Validators.required])),
      id: new FormControl('', Validators.compose([]))
    });

    this.postApprovalDetailsFrm = new FormGroup({
      protocol_primary_batches: new FormControl('', Validators.compose([Validators.required])),
      protocol_commitment_batches: new FormControl('', Validators.compose([Validators.required])),
      protocol_ongoing_batches: new FormControl('', Validators.compose([Validators.required])),
      id: new FormControl('', Validators.compose([]))
    ...
    this.stabilitydataDetailsFrm = new FormGroup({
      ...
      module_3: new FormControl('', Validators.compose([])),
      id: new FormControl('', Validators.compose([]))
  ...
  autoLoadProductsOtherDetails(product_id) {
    ...
    this.OnLoadPostApprovalStability(product_id);
  ...
  onStabilitySummaryPreparing(e) {
    this.functDataGridToolbar(e, this.funcAddStabilitySummary, 'Stability Summary');
  }
  funcAddStabilitySummary() {
    
    this.isStabilitySummaryWinshow = true;
    this.stabilitySummaryDetailsFrm.reset();

  }
  onPostApprovalStabilityPreparing(e) {
    this.functDataGridToolbar(e, this.funcAddPostApprovalStability, 'Post-Approval Stability');
  }
  onStabilityDataPreparing(e) {
    this.functDataGridToolbar(e, this.funcAddStabilityData, 'Stability Data');
  }
  funcAddPostApprovalStability() {
            ...
            .subscribe(
              data_response => {
                ...
                if (resp.success) {
                  ...
                  else if (reload_type == 'post_stability') {
                    this.OnLoadPostApprovalStability(appproduct_id);
...
OnLoadStabilitySummary(product_id) {
    var data = {
      table_name: 'tra_stability_summary',
...
OnLoadPostApprovalStability(product_id) {
    var data = {
      table_name: 'tra_postapproval_stability',
      product_id: product_id
    };
    this.config.getProductsQualitySummaryDetails(data)
      .subscribe(
        data => {
          this.postApprovalStabilityData = data.data;
        });
  }

OnLoadStabilityData(product_id) {
    var data = {
      table_name: 'tra_stability_data',
      product_id: product_id
    };
    this.config.getProductsQualitySummaryDetails(data)
      .subscribe(
        data => {
          this.StabilityDta = data.data;
        });
  }
...
onSaveStabilitySummaryDetails() {
  ...
  this.spinner.show();
  this.appService.onSaveProductQualitySummaryDetails('tra_stability_summary', this.stabilitySummaryDetailsFrm.value, this.product_id,this.application_code)
    .subscribe(
      response => {
        ...
        if (this.product_resp.success) {
          ...
          this.isStabilitySummaryWinshow = false;
...
onSavePostApprovalStabilityDetails() {
  let appproduct_id = this.product_id;
  this.spinner.show();
  this.appService.onSaveProductQualitySummaryDetails('tra_postapproval_stability', this.postApprovalDetailsFrm.value, this.product_id,this.application_code)
    .subscribe(
      response => {
        this.product_resp = response.json();
        //the details 
...
onSaveStabilityDetails() {
  let appproduct_id = this.product_id;
  this.spinner.show();
  this.appService.onSaveProductQualitySummaryDetails('tra_stability_data', this.stabilitydataDetailsFrm.value, this.product_id,this.application_code)
    .subscribe(
      response => {
        this.product_resp = response.json();
        //the details 
