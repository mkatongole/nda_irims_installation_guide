...
@Component({
  selector: 'app-characterisation',
  templateUrl: './characterisation.component.html',
  styleUrls: ['./characterisation.component.css']
})
export class CharacterisationComponent extends SharedProductregistrationclassComponent{
 ...
 // @Input() summaryProductDetailsFrm:FormGroup;
  ...
  isElucidationCharacterisationWinshow:boolean=false;
  isImpurityManufacturerWinshow:boolean=false;
  ...
  elucidationCharacterisationData:any;
  impuritiesData:any;
  criticalControlData:any;

  elucidationCharacterisationDetailsFrm:FormGroup;
  impurityManufacturingDetailsFrm:FormGroup;
 ...
 ngOnInit() { this.onLoadconfirmDataParmAll();
    ...
    this.elucidationCharacterisationDetailsFrm = new FormGroup({
      list_studies_performed: new FormControl('', Validators.compose([Validators.required])),
      isomerism_potentials: new FormControl('', Validators.compose([Validators.required])),
      summary_polymorphic_form: new FormControl('', Validators.compose([Validators.required])),
      Summary_size_distribution: new FormControl('', Validators.compose([])),
      other_characteristics: new FormControl('', Validators.compose([])),
      id: new FormControl('', Validators.compose([]))
    });

    this.impurityManufacturingDetailsFrm = new FormGroup({
      api_related_impurity: new FormControl('', Validators.compose([Validators.required])),
      id: new FormControl('', Validators.compose([]))
  ...
  autoLoadProductsOtherDetails(product_id) {
    this.OnLoadElucidation(product_id);
  ...
  onElucidationrPreparing(e) {
    this.functDataGridToolbar(e, this.funcAddElucidationName, 'Elucidation characterisation');
  }
  funcAddElucidationName() {
    
    this.isElucidationCharacterisationWinshow = true;
    this.elucidationCharacterisationDetailsFrm.reset();

  }
  onImpuritiesPreparing(e) {
    this.functDataGridToolbar(e, this.funcAddImpurities, 'Impurities');
  }
  funcAddImpurities() {
    
    this.isImpurityManufacturerWinshow = true;
    this.impurityManufacturingDetailsFrm.reset();
          ...
          this.appService.onDeleteProductsDetails(record_id, table_name, appproduct_id, title)
            ...
            .subscribe(
              data_response => {
                ...
                if (resp.success) {
                  if (reload_type == 'impurities') {
                    this.OnLoadImpurities(appproduct_id);

                  }
                  else if (reload_type == 'elucidation_characterisation') {
                    this.OnLoadElucidation(appproduct_id);
...
OnLoadElucidation(product_id) {
    var data = {
      table_name: 'tra_elucidation_characterisation',
      product_id: product_id
    };
    this.config.getProductsQualitySummaryDetails(data)
      .subscribe(
        data => {
          this.elucidationCharacterisationData = data.data;
        });
  }

OnLoadImpurities(product_id) {
    var data = {
      table_name: 'tra_impurities_characterisation',
      product_id: product_id
    };
    this.config.getProductsQualitySummaryDetails(data)
      .subscribe(
...
onSaveElucidationCharacterisationDetails() {
  let appproduct_id = this.product_id;
  this.spinner.show();
  this.appService.onSaveProductQualitySummaryDetails('tra_elucidation_characterisation', this.elucidationCharacterisationDetailsFrm.value, this.product_id,this.application_code)
    .subscribe(
      response => {
        ...
        if (this.product_resp.success) {
          ...
          this.isElucidationCharacterisationWinshow = false;
...
onSaveImpurityManufacturingDetails() {
  let appproduct_id = this.product_id;
  this.spinner.show();
  this.appService.onSaveProductQualitySummaryDetails('tra_impurities_characterisation', this.impurityManufacturingDetailsFrm.value, this.product_id,this.application_code)
    .subscribe(
      response => {
        this.product_resp = response.json();
        //the details 
        if (this.product_resp.success) {
          ...
          this.isImpurityManufacturerWinshow = false;
          this.toastr.success(this.product_resp.message, 'Response');
        } else {
          this.toastr.error(this.product_resp.message, 'Alert');
        }
