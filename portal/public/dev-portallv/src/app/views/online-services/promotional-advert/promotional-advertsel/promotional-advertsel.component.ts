...
export class PromotionalAdvertselComponent extends SharedpromotionalAdvertComponent {
  ...
  @Input() promotionalMaterialsfrm: FormGroup;
  ...
  onLoadPromotionalMaterialsDetails() {
    ...
    this.config.onLoadConfigurationData(data)
      .subscribe(
        data => {
          this.promotionalMaterialsData = data;
        });
  }
...
onGenericNameSelected(event){
  ...
  OnSelectAdvertMaterial($event){
      if( $event.selectedItem.id == 10){
        this.is_other_advert_materials = true;
        this.promotionalMaterialsfrm.get('other_advert_materials').setValidators(Validators.required);
        this.promotionalMaterialsfrm.get('other_advert_materials').updateValueAndValidity();
    }
    else{
      this.is_other_advert_materials = false;
      this.promotionalMaterialsfrm.get('other_advert_materials').clearValidators();
      this.promotionalMaterialsfrm.get('other_advert_materials').updateValueAndValidity();
    }
  }
