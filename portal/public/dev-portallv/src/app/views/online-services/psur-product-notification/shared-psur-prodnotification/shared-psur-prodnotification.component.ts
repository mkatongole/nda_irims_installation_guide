...
@Component({
  selector: 'app-shared-psur-prodnotification',
  templateUrl: './shared-psur-prodnotification.component.html',
  styleUrls: ['./shared-psur-prodnotification.component.css']
})
export class SharedPsurProdnotificationComponent implements OnInit {
 ...
 isPrintReportVisible:boolean = false;
  ...
  constructor(public cdr: ChangeDetectorRef,public dmsService:DocumentManagementService, public fb: FormBuilder, public modalServ: ModalDialogService, public viewRef: ViewContainerRef, public spinner: SpinnerVisibilityService, public configService: ConfigurationsService, public appService: PremisesApplicationsService, public router: Router, public formBuilder: FormBuilder, public config: ConfigurationsService, public modalService: NgxSmartModalService, public toastr: ToastrService, public authService: AuthService,public utilityService:Utilities) {
   ...
   // this.onBusinessTypesLoad(this.section_id);
    ...
    if (this.premisesapp_details) {
          ...
          this.productNotificationGeneraldetailsfrm.patchValue(this.premisesapp_details);
  ...
  onSaveProductNotificationApplication() {

    const invalid = [];
    const controls = this.productNotificationGeneraldetailsfrm.controls;
    for (const name in controls) {
        if (controls[name].invalid) {
          this.toastr.error('Fill In All Mandatory fields with (*), missing value on '+ name.replace('_id',''), 'Alert');
            return;
    ...
    if (this.productNotificationGeneraldetailsfrm.invalid) {
      return;
    }
    ...
    this.spinner.show();
    this.appService.onSaveProductNotificationApplication(this.application_id,this.productNotificationGeneraldetailsfrm.value,this.tracking_no)
      .subscribe(
        response => {
          this.premises_resp = response.json();
          //the details 
          ...
          this.tracking_no = this.premises_resp.tracking_no;
          this.application_code = this.premises_resp.application_code;
          this.application_id = this.premises_resp.application_id;
  ...
  onPremisesApplicationSubmit() {
    if(this.sub_module_id == 116){
    this.app_route = ['./online-services/product-nofication-dashboard'];
    this.utilityService.onPermitsApplicationSubmit(this.viewRef, this.application_code, this.tracking_no,'wb_psur_pbrer_applications', this.app_route,this.onApplicationSubmissionFrm.value);
