...
@Component({
  selector: 'app-psur-prodnotification-dashboard',
  templateUrl: './psur-prodnotification-dashboard.component.html',
  styleUrls: ['./psur-prodnotification-dashboard.component.css']
})
export class PsurProdnotificationDashboardComponent implements OnInit {
 app_renewalduenotifications:number=0
  ...
  premises_resp:any;
  dtNotificationApplicationData:any = [];
  ...
  onSelectPremisesFilters(e) {
    ...
    this.reloadPSURApplications({sub_module_id:this.sub_module_id,product_classification_id:product_classification_id,application_status_id:application_status_id});
  ...
  reloadPSURApplications(filter_params={}) {

    this.appService.onPsurNotificationApplicationLoading(filter_params)
      .subscribe(
        resp_data => {
          if (resp_data.success) {
            this.dtNotificationApplicationData =  resp_data.data;
  ...
  onClickSubModuleAppSelection(sub_module_id,sub_module_name){
    ...
    if(sub_module_id == 116){
      ...
      this.app_route = ['./online-services/drug-product-notification'];
      this.router.navigate(this.app_route);
    }

  }
  onLoadNotificationCounterDetails(sub_module_id) {

    this.utilityService.onLoadApplicationCounterDetails('wb_psur_pbrer_applications',sub_module_id)
      .subscribe(
        data => {
          if (data.success) {
...
*/
  ...
  onNotificationToolbarPreparing(e) {
  ...
  funcPremiseArchiveApplication(data){
   this.utilityService.funcApplicationArchiceCall(this.viewRef,data,'wb_premises_applications', this.reloadPSURApplications)
  ...
  funcPremisePreveditDetails(app_data) {
        let application_id = app_data.id;
        this.spinner.show();
        this.appService.getproductNotificationDetails(application_id)
        .subscribe(
          data => {
            this.processData = data.data;
            this.spinner.hide();
            if(data.success){
                  this.title = this.processData.process_title;
                    this.router_link = this.processData.router_link;
                    this.appService.setPremisesApplicationDetail(this.processData);
                    this.appService.getPremisesApplicationDetail();
                    if(this.router_link != ''){
                      this.app_route = ['./online-services/' + this.router_link];
