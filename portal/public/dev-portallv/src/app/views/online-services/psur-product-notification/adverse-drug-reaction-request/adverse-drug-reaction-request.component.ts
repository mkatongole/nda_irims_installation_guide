...
@Component({
  selector: 'app-adverse-drug-reaction-request',
  templateUrl: './adverse-drug-reaction-request.component.html',
  styleUrls: ['./adverse-drug-reaction-request.component.css']
})
export class AdverseDrugReactionRequestComponent extends SharedPsurProdnotificationComponent implements OnInit {
  ...
  app_routing:any;
   ...
   ngOnInit() {
    ...
    if (!this.premisesapp_details) {
      this.router.navigate(['./../online-services/adverse-drug-reaction-dashboard']);
      return;
    }
    else {
