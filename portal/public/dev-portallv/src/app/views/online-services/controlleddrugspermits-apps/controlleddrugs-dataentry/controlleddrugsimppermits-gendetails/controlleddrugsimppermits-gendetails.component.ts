...
@Component({
  selector: 'app-controlleddrugsimppermits-gendetails',
  templateUrl: './controlleddrugsimppermits-gendetails.component.html',
  styleUrls: ['./controlleddrugsimppermits-gendetails.component.css']
})
export class ControlleddrugsimppermitsGendetailsComponent implements OnInit {
  ...
  islicensesSearchWinVisible:boolean;
  @Input() registered_premisesData: any; 
  approvedControlledDrugsPermits:any;
  ...
  has_apppliedctrdrugs_license:boolean;
  ...
  onHasAppliedControlledDrugs($event) {
    if ($event.selectedItem.id == 1) {
      this.has_apppliedctrdrugs_license = true;
    }
    else {
      this.has_apppliedctrdrugs_license = false;
    }
  ...
  onLoadapprovedControlledDrugsPermits() {
    this.spinner.show();
    this.appService.onPermitApplicationLoading('importexportapp/getApprovedControlledDrugsPermits',{sub_module_id:60})
      .subscribe(
        data_response => {
          this.spinner.hide();
          this.islicensesSearchWinVisible = true;
          this.approvedControlledDrugsPermits = data_response.data;
        },
        error => {
          return false
        });
...
*/
  ...
  funcSelectControlledDrugsPermits(data) {

      this.applicationGeneraldetailsfrm.get('controlled_drugslicense_no').setValue(data.data.controlled_drugslicense_no);
      this.applicationGeneraldetailsfrm.get('license_application_code').setValue(data.data.license_application_code);
      this.applicationGeneraldetailsfrm.patchValue(data.data)
      this.islicensesSearchWinVisible = false;
      this.applicationGeneraldetailsfrm.get('sub_module_id').setValue(this.sub_module_id);
      
  }
  
  onSearchControlledDrugsLicense(){
    this.spinner.show();
    this.appService.onPermitApplicationLoading('importexportapp/getApprovedControlledLicensesApplicationLoading',{sub_module_id:60 })
    .subscribe(
      resp_data => {
        if (resp_data.success) {
          this.islicensesSearchWinVisible = true;
          this.approvedControlledDrugsPermits = resp_data.data;
        }
        else {
